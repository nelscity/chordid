...
```bash
cd ~/Developer/chordid
bash install.sh
```

This drops `chordid.amxd` plus the JS dependencies into Live's User Library at:

```
~/Music/Ableton/User Library/Presets/MIDI Effects/Max MIDI Effect/
```

**Windows**: copy `chordid.amxd`, `chord-engine.js`, and `keyboard-display.js` to:
